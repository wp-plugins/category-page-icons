...
function wpdev_cp_define_static() {
    ...
    if (file_exists(WPDEV_CP_PLUGIN_DIR. '/include/wpdev-pro.php')) { require_once(WPDEV_CP_PLUGIN_DIR. '/include/wpdev-pro.php' ); }
}
...
if (!class_exists('wpdev_compose')) {
    class wpdev_compose {
        ...
        function wpdev_compose() {
            ...
            if ( class_exists('wpdev_cp_pro')) {  $this->wpdev_cp_pro = new wpdev_cp_pro(); }
            else {                                $this->wpdev_cp_pro = false;              }
       ...
       function on_show_page_adminmenu($html_id, $icon, $title, $content_type) {
           ?>
            ...
            <h2><?php echo $title; ?></h2>
                <?php
                switch ($content_type) {
                    case 1: $this->content_of_compose_menu_page(); break;
                    case 2: $this->content_of_icons_page(); break;
            ...
            foreach ($my_childs as $pg) {
                ...
                if (! empty ($pg['icon']) ) {
                    ...
                    if ( $pos !== false) {
                        ...
                        $pos = apply_bk_filter('wpdev_pos_img_page',$pos, $output);
                        $beforeImg = apply_bk_filter('wpdev_before_img_page','');
                        $afterImg  = apply_bk_filter('wpdev_after_img_page','');
            ...
            foreach ($my_childs as $pg) {
                ...
                if (! empty ($pg['icon']) ) {
                    ...
                    if ( $pos !== false) {
                        ...
                        $pos = apply_bk_filter('wpdev_pos_img_category',$pos, $output);
                        $beforeImg = apply_bk_filter('wpdev_before_img_category','');
                        $afterImg  = apply_bk_filter('wpdev_after_img_category','');
